...
var IngressAnnotationsitem = []Annotation{
	...
	Annotation{"auth-realm", "区域认证","认证请求的范围：例子auth-realm：Authentication Required - foo"},
	Annotation{"auth-secret", "认证K8s密钥","Secret 的名称，其中包含用户名和密码，这些用户名和密码被授予访问 Ingress 规则中定义的路径的权限。这个注释还接受“ namespace/secretName”的替代形式，在这种情况下，Secret 查找在被引用的名称空间而不是 Ingress 名称空间中执行。"},
	Annotation{"auth-secret-type", "认证密钥类型(auth-file|auth-map)","用来指定Secret的认证类型[auth-file|auth-map]"},
	Annotation{"auth-type", "HTTP身份验证类型(basic|digest)","指示 HTTP 身份验证类型: 基本或 HTTP摘要认证。"},
	Annotation{"auth-tls-secret", "包含完整证书Secret的名称","包含完整证书颁发机构链的 Secret 的名称ca.crt能够对这个入口进行验证。这个注释还接受“ namespace/secretName”的替代形式，在这种情况下，Secret 查找在被引用的名称空间而不是 Ingress 名称空间中执行"},
	Annotation{"auth-tls-verify-depth","认证的深度(Number)"," 提供的客户端证书与证书颁发机构链之间的验证深度"},
	Annotation{"auth-tls-error-page","证书认证错误时候出现的错误页","发生证书认证错误时，应重定向用户的 URL/页面,例子：auth-tls-error-page：http://www.mysite.com/error-cert.html"},
	Annotation{"auth-tls-verify-client","开启验证true/false","允许验证客户端证书"},
	Annotation{"auth-tls-pass-certificate-to-upstream","默认不启用"," 指示是否将接收到的证书传递给上游服务器。默认情况下禁用此选项"},
}
